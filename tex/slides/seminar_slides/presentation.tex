...
% =========================================================================== %
\begin{frame}
\frametitle{Propagating CRUD Through Time:  Flatten-to-Mean Strategy}
\begin{itemize}
\item  If $g(\cdot)$ does not depend on the past CRUD history then $g_i^{t+1} = g(x_i| \mu_g^{t}, \delta t) = g( x_i| g_{i}^{t}, \delta t)$.
\item This leads to the following CRUD propogation scheme:
\begin{itemize}
	\item Draw TH samples, $\{x_i, ... x_N\} \sim h$
	\item Compute the crud distribution over the patch
	$g_i^{t+1} = g(x_i|\mu_g^t, \delta t)$
	\item Identify the ``mean crud flake" on the patch: $g^{t+1}_{i^*}$
	\item Find the boundary conditions which gave rise to $g^{t+1}_{i^*}$, \\\ denote them $x^*$.  Set all boundary condition samples: $x^*_i = x^*, \forall i \in {1, ... N}$
	\item Rewind the CRUD simulation back to $g^{t}$
	\item Regrow crud with uniform TH boundary conditions over the patch:  $g_i^{t+1} = g(x^*_i|\mu_g^t, \delta t)$
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item  \textbf{Problem:}  The crud growth rate has some dependence on the previous crud history.  
