...
To test the method on a single pin and for a single state, CTF was executed with boundary conditions and the power level shown in table ().  Next, a synthetic CFD data set was generated  from $ctfPurt$ (see \autoref{chap:synth} for details).
The gradient boosted copula model was then fit to the synthetic single pin CFD data set.  Finally, the model was evaluated on each CTF patch on the pin surface and the CRUD model was executed [in a monte-carlo-fashion on each CTF patch with 200 samples/patch] given boundary conditions sampled from the boosted model. The CRUD results were then summed over the rod surface and are presented in table ().

\begin{figure}[hbtp]
\centering
\includegraphics[width=10cm]{images/ktau_plot.png}
\caption{Kendall's tau vs. axial position. Output from combined regression and classification tree evaluation. Colored by copula type: Green=Gauss, Blue=Gumble, Red=Frank.}
\label{fig:ktau_plot}
\end{figure}
