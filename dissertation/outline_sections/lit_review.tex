...
Where the first term is accounts for the effect of grid flow restriction and the second term represents the contribution of vane induced swirl on the heat transfer.
    ...
    Where $\dot m_i$ is the inlet mass flow rate.  The superscript, $(\cdot)^l$, represents low flow conditions and $(\cdot)^h$ represent high flow conditions.
     ...
     \item Despite high span to span repeatability, areas of over or under prediction by ($\pm 3[K]$)  are present in the post-hi2lo temperature profiles.  This detracts from the CILC predictive power of theis hi2lo method since CILC is likely to occur at outlying hot spots.  It is difficult to predict the magnitude and location of hot spots on the rod surface.
     \item The problem may recast in terms of CILC risk.  Rather than attempting to predict how much and where CILC will occur, one may ask: What is the expected fraction of the rod surface that exceeds some critical temperature?
     ...
     \item In cases with significant subcooled boiling the average wall superheat predicted by CFD and CTF do not agree.  This could be due to a mis-calibration of the boiling models employed in CFD or in CTF.
