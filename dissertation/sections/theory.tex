...
Algorithm \ref{algo:basic_crud_algo} is used to compute the total crud mass in each CTF face.

\begin{algorithm}[H]
    \captionsetup{labelfont={sc,bf}, labelsep=newline}
    \caption{Generic statistically based hi2lo method for crud prediction.}
    \begin{algorithmic}        
    \STATE (1) Pre-process training set $\theta(\mathbf p, \mathbf z)$.  
    \STATE (2) Train model $\mathcal M(\mathbf p ; \gamma)$:  $ argmin_\gamma ||\mathcal M(\mathbf p; \gamma) - \theta(\mathbf p)|| $
\FOR {CTF face, $i$}
    \STATE Evaluate ML model $\hat \theta_i \leftarrow \mathcal M(\mathbf p_{i})$ \;
    \STATE Reconstruct $\hat H_i(\cdot |\hat \theta_i)$ \;
    \STATE Draw samples $\mathbf X \sim \hat H_i$ \;
...
The sample time is denoted by $s$:
\begin{equation}
s_n = n\Delta t_s
\end{equation}

The time averaged sample weights are updated at every resampling event according to:

\begin{equation}
\bar \omega = \left( \frac{1}{\sum \Delta t_s} \right) \sum_{i=0}^n \omega_i \Delta t_{s_i}
\end{equation}
And $\omega_i$ is computed according to the ratio of the target density and the sampling distribution density as given in equation ().
