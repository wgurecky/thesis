...
\subsection{Stepping Scheme}

In figure \ref{fig:stepscheme}, the VERA state-point time index is denoted by $t$. Resampling events are performed at time intervals $\Delta s$.  At each resampling event $n$, samples are drawn from the reconstructed joint probability $\hat H^t$.  Assume that the sample time step is constant and equal to $\Delta s$.
