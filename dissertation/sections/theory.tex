...
Here a non parametric representation of univariate distributions is given by considering a set of sample quantiles.  
...
Alternatively, one may construct a non-parametric univariate density given some set of predictive features by utilizing a set of predicted distribution cumulants.  The estimated cumulants, produced by evaluating some trained model, can be used to build an Edgeworth series expansion \cite{hall1997}.  It remains as future work to determine if cumulants or traditional moments behave in a predictable manner as a function of local core conditions.
...
It can be shown that the sample quantiles are distributed according to \ref{eq:theory_qdist_1} \cite{mosteller1946}.  This directly follows from the distribution of the order statistics.  The theoredical distribution of the quantiles is Gaussian in the large sample limit.  When benchmarking or validating quantile predictions it is essential to check that the predictions follow the expected Gaussian behavior.  For the quantile regression procedures employed in this work, this check is performed in section \ref{chap:GBRT}.  In the current application it is important to estimate uncertainties inherent to sample quantiles so such uncertainties may be propogated into the crud integration procedures.
