...
Let the quantile function be represented by $Q=F^{-1}$ where $F$ is a cumulative density function.
...
In order to estimate a sample quantile, $\hat q_\tau$, given the empirical CDF $F$, minimize: $\E[\rho_\tau(T - q_\tau)]$ where $T$ is a random variable distributed according to $F_T$.
\begin{equation}
            \left.\begin{aligned}
...
In this work, a set of sample quantiles are used to construct a step-wise cumulative distribution, which in turn is used to build a histogram.  In place of the stepwise representation, a piecewise cubic hermite interpolating polynomial (PHCIP) can be fit to the stepwise conditional quantile distribution to generate a differentiable CDF.
The PCHIP interpolation preserves monotonicity of the CDF if the provided quantiles are strictly monotone.  This condition is enforced in the software; any violation of the monotone restriction would indicate a software bug in the quantile regression code.  Inverse transform sampling is used to draw samples from the univariate distributions.

Other strategies to model univariate density functions are abundant.  One alternative is to use a set of sample moments.  Next, a set of moment conditions is defined for the target parametric model.  Then the method of moments can be used to fit the parametric model to sample moments.  Non-parametric extension of the method of moments exist (refs).  
...
\begin{equation}
F(\mathbf p; \mathbf{\beta}, \mathbf a) = \sum_{m=0}^M \beta_m h(\mathbf p; a_m)
\label{eq:single_cart_model}
\end{equation} 
Where $M$ is the number of constituent sub-models and $\{ \beta_m, a_m\}$ are coefficients and free parameters requiring fitting in each sub-model.
    ...
    \begin{algorithmic}      
        ...
        \FOR {CTF face, $j$}
          ...
          \STATE Reconstruct margins (CDFs) from quantiles  \\
            $\ \ F_T= Q^{-1}(T; \{\hat{\theta}_\tau\})$ , $F_k= Q^{-1}(k; \{\hat{\theta}_\tau\})$
          \STATE Def $q''$ margin  $f_{q''} = \delta_{(q''_\mathrm{j,ctf})}$
          \STATE Reconstruct joint distribution $\hat h_j(\cdot |\hat \theta_j) = \hat f_T \hat f_k f_{q''} c(T, k; \hat \theta_c)$ \;
        \ENDFOR
          \FOR {Resample time step, $\Delta t_s$}
            \FOR {CTF face, $j$}
              \STATE Def importance sampling distribution $\tilde h = \tilde f_T(T; \beta_T) \tilde f_k(k; \beta_k)c(T, k; \hat \theta_c) $
...
\begin{table}[h]
    \begin{center}
        ...
        \begin{tabular}[h]{|l | l | L | }
            ...
            \hline
            $\omega_T$  & 0.6 & Temperature weighting factor used to remap samples on the rod surface. \\ 
            \hline
            $\beta_T$  & $\{0.9, 1.0 \}$ & Beta distribution parameters for temperature importance distribution. \\ 
            \hline
            $\beta_k$  & $\{0.9, 0.8 \}$ & Beta distribution parameters for TKE importance distribution. \\ 
            \hline
        \end{tabular}
