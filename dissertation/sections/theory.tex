...
\begin{algorithm}[H]
    ...
    \STATE (2) Train model $\mathcal M(\mathbf p, \mathbf z ; \gamma)$:  $ argmin_\gamma ||\mathcal M(\mathbf p, \mathbf z; \gamma) - \theta(\mathbf p, \mathbf z)|| $
\FOR {CTF face, $i$}
    \STATE Evaluate ML model $\hat \theta_i \leftarrow \mathcal M(\mathbf p_{i}, \mathbf z_i; \gamma)$ \;
