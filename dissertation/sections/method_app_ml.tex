...
Where $n=20$ in this case since the predicted CDFs are supported at 20 locations. The number of samples available to construct the empirical distribution, $m$, on each axial edit was $\approx800$, though this varied slightly from face to face.  In general $c(\alpha) = \sqrt{-\frac{1}{2} \mathrm{ln}\alpha}$, therefore at $\alpha=0.1$, $KS_D^* \approx 0.243$.  A summary of the KS distances and test results are provided in table \ref{tab:ks_temp} for the temperature quantiles and in table \ref{tab:ks_tke} for the predicted TKE quantiles.
