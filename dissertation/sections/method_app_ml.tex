...
A principal goal of the machine learning model is to predict the conditional quantiles of the temperature and TKE distributions as a function of local core conditions.  In this light, the trained quantile regression models are compared against the left-out CFD data set on each pin.  The accuracy of both the TKE and tempearture quantile regressors is assessed using both quantile-quantile plots and quantile vs axial rod position comparisons.
...
Shown in the axial plots, the TKE distribution is drastically influenced by spacer grids.  The maximum near-wall TKE sharply increases following a spacer grid followed by a decay towards a more orderly flow.  Also notable is the location of minimum predicted near wall TKE which immediately follows spacer grids.  In addition to increasing the net turbulent kenetic energy of the flow, mixing vanes can also produce eddy regions of realative stagnant flow thus giving rise to these regions of low near wall TKE.  The Hi2lo model retains both of these properties of the flow field resolved by CFD.  The prediction accuarcy of the quantile regressors as measured by LOO cross validation is striking considering the limited 24 pin training set used in these cases.  Good overall performance of the machine lerning models to predict the quantiles of the TKE distributions can be attributed to high pin-to-pin similarity of the surface TKE distributions. High span to span repeatability of the TKE distributions were also observed.  This observation is consistent with those found in other hi2lo studies by Salko et. al \cite{salko18}.

Temperature residual quantile regression results are given in figures \ref{fig:temppin1} to \ref{fig:temppin3}.  Similar to the TKE conditional quantiles, the conditional temperature distribution exhibits sharp changes in behavior across the spacer grids.  Discontinuities enforced the choice of the gradient boosted tree machine learning algorithm which is resilient to steep gradients in the response surface.
...
Inspecting the axial quantile difference figures \ref{fig:temppin1}a to \ref{fig:temppin3}a, on average the extreme quantiles exhibit the largest axial RMS prediction errors.  This is expected behavior since the estimates the extreme quantiles from a sample population is naturally fraught with high variance as described by equation \ref{eq:theory_qdist_1}.  This fact was experimentally demonstrated using a simple test quantile regression problem in section \ref{chap:GBRT}.
...
Stacking increases model complexity since each classifier in the ensemble contains hyperparameters which require tuning.
...
The RMS axial crud distribution errors are sumarrized in table \ref{tab:loo_rms}.  To facilitate examination of the hi2lo model predictions for gemoetric biases across the assembly top-down views of the RMS crud mass and boron mass error distributions were generated and presented in figure \ref{fig:l2boronasmerrorshmap} and \ref{fig:l2cmassasmerrorshmap} respectively.  The pins which reside on the edge of the assembly did not exhibit any increase in crud error on average.  The root mean squared error is given by: $RMS = \sqrt{\sum_j(\mathrm{Hi2lo}_j - \mathrm{CFD}_j)^2}$ where $j$ is the CTF face index on a given pin.  
